...
// Si el admin no tiene club asignado, puede ver todos los equipos y jugadores.
$esAdminGlobal = empty($club_id);

if ($esAdminGlobal) {
    $sqlEquipos = "SELECT id, nombre, categoria FROM equipos ORDER BY nombre ASC";
    $stmtEquipos = $pdo->query($sqlEquipos);
...
if (!$esAdminGlobal) {
    $sql .= " AND e.equipo_id = :club_id";
    $params[':club_id'] = $club_id;
}

if ($equipoSeleccionado > 0) {
    $sql .= " AND e.id = :equipo_id";
    $params[':equipo_id'] = $equipoSeleccionado;
}

$sql .= " ORDER BY e.categoria ASC, j.posicion DESC";

$stmtJugadores = $pdo->prepare($sql);
