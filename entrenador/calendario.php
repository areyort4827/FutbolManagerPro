...
$equipo_id = $_SESSION['equipo_id'] ?? 0;
...
$sqlEventos = "SELECT COUNT(*) FROM entrenamientos 
               WHERE club_id = :club_id AND equipo_id = :equipo_id
                 AND YEAR(fecha) = :anio 
                 AND MONTH(fecha) = :mes";
...
$sqlEntrenamientos = "SELECT COUNT(*) FROM entrenamientos WHERE club_id = :club_id AND equipo_id = :equipo_id";
$stmt = $pdo->prepare($sqlEntrenamientos);
$stmt->execute(['club_id' => $club_id,'equipo_id' => $equipo_id]);
