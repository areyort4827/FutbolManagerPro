...
// ── Partidos: totales, victorias, empates, derrotas y GOLES DEL CLUB ──────────
$sqlPartidos = "
    SELECT 
        COUNT(*) AS total,
        SUM(CASE 
            WHEN p.equipo_local_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id) 
                 AND CAST(SUBSTRING_INDEX(p.resultado,'-',1) AS UNSIGNED) > CAST(SUBSTRING_INDEX(p.resultado,'-',-1) AS UNSIGNED) THEN 1
        ...
        /* CÁLCULO CORRECTO DE GOLES A FAVOR (Goles marcados por mi club) */
        SUM(CASE 
            WHEN p.equipo_local_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id3) THEN CAST(SUBSTRING_INDEX(p.resultado,'-',1) AS UNSIGNED)
            WHEN p.equipo_visitante_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id4) THEN CAST(SUBSTRING_INDEX(p.resultado,'-',-1) AS UNSIGNED)
            ELSE 0 END) AS mis_goles_favor,

        /* CÁLCULO CORRECTO DE GOLES EN CONTRA (Goles recibidos por mi club) */
        SUM(CASE 
            WHEN p.equipo_local_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id5) THEN CAST(SUBSTRING_INDEX(p.resultado,'-',-1) AS UNSIGNED)
            WHEN p.equipo_visitante_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id6) THEN CAST(SUBSTRING_INDEX(p.resultado,'-',1) AS UNSIGNED)
            ELSE 0 END) AS mis_goles_contra

    FROM partidos p
    WHERE p.resultado IS NOT NULL AND p.resultado != ''
      AND (p.equipo_local_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id7)
        OR p.equipo_visitante_id IN (SELECT id FROM equipos WHERE equipo_id = :club_id8))";
...
// Variables de goles corregidas
$goles_favor   = (int)($statsPartidos['mis_goles_favor'] ?? 0);
$goles_contra  = (int)($statsPartidos['mis_goles_contra'] ?? 0);
    ...
    <script>
        ...
        new Chart(document.getElementById('rendimientoChart').getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                        label: 'Victorias',
                        data: <?= json_encode($data_victorias) ?>,
                        backgroundColor: '#10b981'
                    },
                    {
                        label: 'Empates',
                        data: <?= json_encode($data_empates)   ?>,
                        backgroundColor: '#eab308'
                    },
                    {
                        label: 'Derrotas',
                        data: <?= json_encode($data_derrotas)  ?>,
        ...
        new Chart(document.getElementById('golesChart').getContext('2d'), {
            ...
            data: {
                ...
                datasets: [{
                        label: 'Goles a favor',
                        data: <?= json_encode($data_goles_favor)  ?>,
                        borderColor: '#10b981',
                    ...
                    },
                    {
                        ...
                        data: <?= json_encode($data_goles_contra) ?>,
                        borderColor: '#ef4444',
