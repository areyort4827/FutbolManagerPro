...
// Todos los equipos del club para el select
$stmt_equipos = $pdo->prepare("SELECT id, nombre, categoria FROM equipos WHERE equipo_id = :club ORDER BY nombre ASC");
$stmt_equipos->execute([':club' => $club_id]);
...
<div class="fichajes-contenedor">
    ...
    <div class="fichajes-header">
        <div>
            ...
            <span>Jugadores libres disponibles — selecciona el equipo destino</span>
        </div>
        ...
        <?php else: ?>
            ...
            <div class="fichajeCard">
                ...
                <form method="POST" action="fichar_jugador.php"
                      onsubmit="return validarFichaje(this, '<?= htmlspecialchars($j['nombre'], ENT_QUOTES) ?>')">
                    <input type="hidden" name="jugador_id" value="<?= $j['id'] ?>">
                    <select name="equipo_id" class="select-equipo" required>
...
<script>
function validarFichaje(form, nombre) {
    const select = form.querySelector('select[name="equipo_id"]');
    if (!select.value) {
        alert('Selecciona un equipo antes de fichar a ' + nombre + '.');
        return false;
    }
    return confirm('¿Fichar a ' + nombre + ' para ' + select.options[select.selectedIndex].text + '?');
}
</script>
