...
if (isset($_POST['guardar_estadisticas'])) {
    ...
    $stmt_fecha = $pdo->prepare("SELECT fecha FROM partidos WHERE id = :id");
    $stmt_fecha->execute([':id' => $partido_id]);
    $fecha_partido = $stmt_fecha->fetchColumn();

    if ($fecha_partido > date('Y-m-d')) {
        echo "<script>alert('No puedes añadir resultado antes de que se juegue el partido'); window.history.back();</script>";
        exit();
    }
    ...
    if ($partido_id > 0) {
        ...
        $stmt_partido = $pdo->prepare("SELECT equipo_local_id FROM partidos WHERE id = :id");
        $stmt_partido->execute([':id' => $partido_id]);
        $equipo_local_id = (int)$stmt_partido->fetchColumn();

        $stmt_club = $pdo->prepare("SELECT equipo_id FROM equipos WHERE id = :id");
        $stmt_club->execute([':id' => $equipo_local_id]);
        $club_local = (int)$stmt_club->fetchColumn();

        $mis_goles_resultado = ($club_local == $club_id) ? (int)$partes[0] : (int)$partes[1];
        ...
        foreach ($_POST['jugadores'] as $stats) {
            ...
            $suma_asistencias += (int)($stats['asistencias'] ?? 0);
        }

        if ($suma_goles_jugadores !== $mis_goles_resultado) {
            echo "<script>alert('Error: El resultado indica $mis_goles_resultado goles para tu club, pero has sumado $suma_goles_jugadores en la lista de jugadores.'); window.history.back();</script>";
            exit();
        }

        if ($suma_asistencias > $suma_goles_jugadores) {
            echo "<script>alert('Error: Las asistencias no pueden ser mayores que los goles.'); window.history.back();</script>";
            exit();
        }

        $stmt_ids = $pdo->prepare("SELECT j.id FROM jugadores j INNER JOIN equipos e ON j.equipo_id = e.id WHERE e.equipo_id = :cid");
        $stmt_ids->execute([':cid' => $club_id]);
        $ids_jugadores = array_column($stmt_ids->fetchAll(PDO::FETCH_ASSOC), 'id');
...
/* ===== JUGADORES DEL CLUB PARA EL MODAL ===== */
$stmt_jug_modal = $pdo->prepare("
    SELECT 
        j.id, 
        j.nombre, 
        j.posicion, 
        j.equipo_id, -- Este es el que necesitamos para el filtro
        e.nombre AS equipo_nombre 
    FROM jugadores j 
    INNER JOIN equipos e ON j.equipo_id = e.id 
    WHERE e.equipo_id = :cid 
    ORDER BY e.nombre ASC, j.nombre ASC
");
$stmt_jug_modal->execute([':cid' => $club_id]);
...
<div id="proximos" class="contenido-tab">
        ...
        <div class="partido-card">
            ...
            <div class="resultado">
                ...
                <div style="display:flex; gap:10px; margin-top:12px;">
                    ...
                    <button type="button" class="btn-stats" onclick="abrirModalStats( <?= $fila['id'] ?>,
    ...
    '<?= htmlspecialchars($fila['resultado'] ?? '', ENT_QUOTES) ?>',
     <?= ($fila['equipo_local_id'] == $fila['local_id']) ? $fila['local_id'] : $fila['visitante_id'] ?>,
    <?= $fila['local_id'] ?>,
    <?= $fila['visitante_id'] ?>
                            )">
    ...
    <div class="modal-contenido">
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <select name="equipo_club_id" id="equipo_club_id" onchange="filtrarRivalYJugadores()" required>
            ...
            <div class="form-group">
                ...
                <select name="rival" id="rival_select" required>
            ...
            <div class="stats-table-wrap">
                <table class="stats-table">
                    ...
                    <tbody>
                        <?php
                        $equipo_actual = null;
                        foreach ($jugadores_modal as $jug):
                            if ($jug['equipo_nombre'] !== $equipo_actual):
                                $equipo_actual = $jug['equipo_nombre'];
                        ?>
                        <!-- Fila de encabezado para cada equipo, con data-equipo-header-id para identificar a qué equipo pertenece -->
                        <tr class="stats-equipo-header" data-equipo-header-id="<?= $jug['equipo_id'] ?>">
                            <td colspan="6"><i class="fa-solid fa-shirt"></i> <?= htmlspecialchars($equipo_actual) ?>
                            </td>
                        </tr>
                        <?php endif; ?>

                        <!-- Fila de jugador, con data-equipo-id para mostrar/ocultar según el equipo seleccionado -->
                        <tr class="fila-jugador-stats" data-equipo-id="<?= $jug['equipo_id'] ?>">
