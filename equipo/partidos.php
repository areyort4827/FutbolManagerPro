...
$stmt_jug_modal = $pdo->prepare("SELECT j.id, j.nombre, j.posicion, e.nombre AS equipo_nombre FROM jugadores j INNER JOIN equipos e ON j.equipo_id = e.id WHERE e.equipo_id = :cid ORDER BY e.nombre ASC, j.nombre ASC");
...
.stats-resultado-input {
    ...
    font-size: 16px; font-weight: 700; width: 120px; transition: 0.2s;
}
...
function abrirModalStats(partidoId, nombre, resultado) {
    ...
    document.querySelectorAll('#formStats .stats-num-input').forEach(i => { i.value = 0; });

    fetch('get_estadisticas.php?partido_id=' + partidoId + '&club_id=<?= $club_id ?>')
        .then(r => r.json())
        .then(data => {
            data.forEach(row => {
