...
<head>
    ...
    <title>Futbol Manager Pro – Jugador</title>
    ...
    <style>
        ...
        .logo { font-size: 18px; font-weight: bold; padding: 20px; color: #22c55e; border-bottom: 1px solid #1f2937; }

        .menu { display: flex; flex-direction: column; padding: 10px; gap: 5px; flex: 1; }

        .menu a {
            text-decoration: none; color: #cbd5e1; padding: 12px 15px; border-radius: 8px;
            display: flex; align-items: center; gap: 12px; cursor: pointer; transition: .2s;
        }
        .menu a:hover { background: #1e293b; color: white; }
        .menu a.active { background: #22c55e; color: #0f172a; font-weight: bold; }

        .user-box {
            margin-top: auto; padding: 16px 20px; border-top: 1px solid #1f2937;
            display: flex; align-items: center; gap: 12px;
        }
        .user-box i { font-size: 38px; color: #22c55e; flex-shrink: 0; }
        .user-info { flex: 1; }
        .user-info strong { display: block; font-size: 15px; margin-bottom: 3px; }
        .role { display: inline-block; padding: 4px 10px; border-radius: 20px; font-size: 12px; background: #16a34a; font-weight: 600; }

        .logout {
            padding: 16px 20px; color: #ef4444 !important; text-decoration: none;
            display: flex; align-items: center; gap: 12px; border-top: 1px solid #1f2937; transition: .2s;
        }
...
<div class="sidebar">
    ...
    <div class="menu">
        <a class="<?= $paginaActual === 'dashboard'       ? 'active' : '' ?>" onclick="mostrarPagina('dashboard')">
            <i class="fa-solid fa-gauge"></i> Dashboard
        </a>
        <a class="<?= $paginaActual === 'jugadores'       ? 'active' : '' ?>" onclick="mostrarPagina('jugadores')">
            <i class="fa-solid fa-users"></i> Mi Equipo
        </a>
        <a class="<?= $paginaActual === 'entrenamientos'  ? 'active' : '' ?>" onclick="mostrarPagina('entrenamientos')">
            <i class="fa-solid fa-dumbbell"></i> Entrenamientos
        </a>
        <a class="<?= $paginaActual === 'partidos'        ? 'active' : '' ?>" onclick="mostrarPagina('partidos')">
            <i class="fa-solid fa-futbol"></i> Partidos
        </a>
        <a class="<?= $paginaActual === 'estadisticas'    ? 'active' : '' ?>" onclick="mostrarPagina('estadisticas')">
            <i class="fa-solid fa-chart-line"></i> Estadísticas
        </a>
        <a class="<?= $paginaActual === 'calendario'      ? 'active' : '' ?>" onclick="mostrarPagina('calendario')">
