...
<style>
.entrenamientosContenedor { padding: 20px; }
.entrenamientosHeader { display: flex; justify-content: space-between; align-items: center; margin-bottom: 22px; flex-wrap: wrap; gap: 12px; }
.entrenamientosHeader h2 { margin: 0; font-size: 22px; }
.estadisticas { display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; margin-bottom: 24px; }
.estadisticasCard { border: 2px solid #bbf7d0; border-radius: 12px; padding: 18px; background: #fff; text-align: center; }
.estadisticasCard .val { font-size: 2rem; font-weight: 700; color: #16a34a; }
.estadisticasCard .lbl { font-size: 13px; color: #6b7280; margin-top: 4px; }
.tabla-wrapper { background: white; border-radius: 14px; box-shadow: 0 4px 14px rgba(0,0,0,0.07); overflow: hidden; }
.tabla { width: 100%; border-collapse: collapse; }
.tabla th { background: #16a34a; color: white; padding: 14px 16px; text-align: left; font-size: 13px; }
.tabla td { padding: 14px 16px; border-bottom: 1px solid #f1f5f9; font-size: 14px; color: #374151; }
.tabla tr:hover td { background: #f0fdf4; }
.tabla tr:last-child td { border-bottom: none; }
.badge-titulo { display: inline-block; padding: 4px 10px; border-radius: 8px; font-size: 12px; font-weight: 600; }
.tactica  { background: #dbeafe; color: #1d4ed8; }
.tecnica  { background: #ede9fe; color: #7c3aed; }
.pre-partido { background: #fef3c7; color: #92400e; }
.fisico   { background: #dcfce7; color: #166534; }
.asistio-si  { color: #16a34a; font-weight: 600; }
.asistio-no  { color: #ef4444; font-weight: 600; }
.asistio-nd  { color: #94a3b8; }
.asistio-prox { color: #2563eb; font-weight: 600; }
.asist-checkbox { transform: scale(1.2); cursor: pointer; }
.asist-form { display: inline-flex; align-items: center; gap: 10px; }
.no-data { text-align: center; color: #94a3b8; padding: 50px; }
@media(max-width:640px){ .estadisticas{ grid-template-columns:1fr; } }
</style>
    ...
    <div class="tabla-wrapper">
        ...
        <table class="tabla">
            ...
            <tbody>
                ...
                <?php
                    $claseT = 'tactica';
                    if (str_contains($e['titulo'], 'tÃ©cnica'))     $claseT = 'tecnica';
                    if (str_contains($e['titulo'], 'pre-partido')) $claseT = 'pre-partido';
                    if (str_contains($e['titulo'], 'fÃ­sico'))      $claseT = 'fisico';
                ...
                <tr>
                    ...
                    <td>
                        <form class="asist-form" action="guardar_asistencia.php" method="POST" style="margin-bottom:6px;">
                            <input type="hidden" name="entrenamiento_id" value="<?= (int)$e['id'] ?>">
                            <label>
                                <input class="asist-checkbox" type="checkbox" name="asistio" <?= ((int)($e['asistio'] ?? 0) === 1) ? 'checked' : '' ?> onchange="this.form.submit()">
